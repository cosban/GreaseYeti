...
if(typeof GM_setValue != 'function' || typeof GM_getValue != 'function' ||
  ...
  alert(
    'Error: You need GM_setValue, GM_getValue, and GM_xmlhttpRequest functions to use GreaseYETI.'
  );
}
// Set some core variables
var url = document.location.href;
var load_gif =
  'data:image/gif;base64,R0lGODlhEAAQAPIAAP///2Zm/9ra/o2N/mZm/6Cg/rOz/r29/iH/C05FVFNDQVBFMi4wAwEAAAAh/hpDcmVhdGVkIHdpdGggYWpheGxvYWQuaW5mbwAh+QQJCgAAACwAAAAAEAAQAAADMwi63P4wyklrE2MIOggZnAdOmGYJRbExwroUmcG2LmDEwnHQLVsYOd2mBzk' +
...
if(url.indexOf('loser.php?settings') != -1) {
  ...
  $(document)
    .on('change', '.highlight_color, #my_color', function () {
      $(this)
        .parent()
        .parent()
        .children()
    ...
    .addEventListener('click', function () {
      ...
      $('tr.huser')
        .each(function () {
          ...
          thisuser.username = $(this)
            .find('.highlight_username')
            .val();
          thisuser.color = $(this)
            .find('.highlight_color')
            .val();
          thisuser.posts = $(this)
            .find('.highlight_posts')
            .is(':checked');
          thisuser.topics = $(this)
            .find('.highlight_topics')
            .is(':checked');
      ...
      $('tr.iguser')
        .each(function () {
          var thisuser = new Object();
          thisuser.username = $(this)
            .find('.ignorate_username')
            .val();
          thisuser.userid = $(this)
            .find('.ignorate_userid')
            .val();
      ...
      $('tr.igkword')
        .each(function () {
          ...
          kword.word = $(this)
            ...
            .val();
          kword.is_regex = $(this)
            .find('.kw_ignorate_regex')
            .is(':checked');
          kword.posts = $(this)
            .find('.kw_ignorate_posts')
            .is(':checked');
          kword.topics = $(this)
            .find('.kw_ignorate_topics')
            .is(':checked');
          greaseyeti.ignorated_kwords.push(kword);
        });
      saveGreaseyeti(false);
      $('span#save_confirmation')
        .html('Settings saved!');
    });
  // ****************
  // * USER PROFILE *
  // ****************
} else if(url.indexOf('profile.php') != -1 && document.body.innerHTML.indexOf(
    'Edit My Profile') != -1) {
  $('.grid')
    .append(
      '<tr><td colspan="2"><a href="loser.php?settings">Edit GreaseYETI Settings</a></td></tr>'
    );
...
if(document.location.href.indexOf('.net/topics/') != -1) {
  ...
  var next_page_link = $('div.infobar')
    .eq(-2)
    .find('a');
  if(next_page_link.length > 0) {
    next_page = next_page_link.eq(0)
      .attr('href');
  } else {
    next_page = false;
  }
  ...
  if(ch('page_change')) {
    ...
    var new_page_observer = new MutationObserver(function (mutations) {
      var new_page_number = parseInt($('#u0_3 a')
        .last()
        .text());
  ...
  if(ch('expand_images')) {
    $(document)
      .on('click',
        '.greaseyeti_img_collapsed, .greaseyeti_img_expanded',
        function (e) {
          toggleImage($(this));
          e.preventDefault();
        });
    if(ch('expand_all')) {
      $('.userbar')
        .append(
          ' | <a class="greaseyeti_link" id="greaseyeti_expand_all">Expand All Images</a>'
        );
      $('#greaseyeti_expand_all')
        .click(expandAllImages);
    }
  }
  ...
  GM_xmlhttpRequest({
    ...
    onload: function (responseDetails) {
      if(responseDetails.status == 200) {
        ...
        var linkct = 0;
        while(dl.match(/\[http(.+?)\]/)) {
          dl = dl.replace(/\[http(.+?)\]/, '<a href="http$1">[' + (++
            linkct) + ']</a>');
        }
        var dl_bgcolor,
          dl_textcolor;
        ...
        case 'rainbow':
          dl_bgcolor =
            'linear-gradient(to right, rgba(255, 50, 50, 0.7), rgba(255, 160, 0, 0.7), rgba(255, 255, 100, 0.7), rgba(150, 255, 150, 0.7), rgba(120, 170, 255, 0.7), rgba(175, 0, 175, 0.7))';
          dl_textcolor = '#000';
          break;
        default:
        ...
        $('.userbar')
          .eq(0)
          .before(
            '<div id="dramalinks_ticker" style="padding: 4px"><strong>' +
            color + ':</strong>' + dl + '</div>');
        $('#dramalinks_ticker')
          .css('background', dl_bgcolor);
        $('#dramalinks_ticker, #dramalinks_ticker a')
          .css('color',
            dl_textcolor);
      }
    }
...
function addCustomCss() {
  ...
  $('head')
    .append('<style type="text/css">' + ch('custom_css_rules') +
      '</style>');
}
  ...
  $('textarea[name=message]')
    .keydown(function (e) {
      ...
      case 85:
        ...
        break;
      case 77:
        if(!open_tags['spoiler']) var caption = prompt(
          'Please enter a caption for your spoiler tag.');
        tag = 'spoiler';
        break;
      default:
        return;
      ...
      if(selectStart == selectEnd) {
        ...
        $(this)
          .val(v.substr(0, selectStart) + ins + v.substr(selectStart));
        $(this)[0].setSelectionRange(selectStart + ins.length, selectStart +
          ins.length);
      } else {
        var open_tag = tag;
        if(caption != null) open_tag = 'spoiler caption="' + caption + '"';
        $(this)
          .val(v.substr(0, selectStart) + '<' + open_tag + '>' + v.substr(
            selectStart, selectEnd - selectStart) + '</' + tag + '>' + v.substr(
            selectEnd));
        $(this)[0].setSelectionRange(selectStart + 2 + open_tag.length,
          selectStart + 2 + open_tag.length + (selectEnd - selectStart));
      }
      e.preventDefault();
    });
}
...
function lastFMListeners() {
  ...
  checkLastFM();
  $('input[value="Post Message"], input[value="Preview Message"]')
    .click(
      function () {
        $(this)
          .parents('form')
          .find('textarea')
          .val(function (i, v) {
            var d = new Date(ch('lastfm_timestamp', '-1'))
              .getTime();
            var now = new Date();
            var stamp = 'Just now';
            if(now - d > 5 * 60000) stamp = Math.floor((now - d) / 60000) +
              'm ago';
            if(now - d > 3600000) stamp = Math.floor((now - d) / 3600000) +
              'h ago';
            if(now - d > 86400000) stamp = Math.floor((now - d) / 86400000) +
              ' days ago';
...
function checkLastFM() {
  if(ch('lastfm_username')
    .length < 1) return;
  if(start / 1000 > (ch('lastfm_lastcheck', 0) + ch('lastfm_freq') * 60)) {
    GM_xmlhttpRequest({
      method: 'GET',
      url: 'http://ws.audioscrobbler.com/1.0/user/' + ch('lastfm_username') +
        '/recenttracks.rss',
      headers: {
      ...
      onload: function (responseDetails) {
        if(responseDetails.status == 200) {
          ...
          var xmlDoc = parser.parseFromString(responseDetails.responseText,
            'text/xml');
          greaseyeti.lastfm_track = xmlDoc.getElementsByTagName('title')[
            1].childNodes[0].nodeValue;
          greaseyeti.lastfm_lastcheck = Math.round(start / 1000);
          greaseyeti.lastfm_timestamp = xmlDoc.getElementsByTagName(
            'pubDate')[1].childNodes[0].nodeValue;
          saveGreaseyeti(true);
        }
      }
    });
    ...
    $('textarea[name=message]')
      .each(function () {
        if($(this)
          .val()
          .split(' ')
          .length > 50) stop_it = true;
      });
    if(stop_it) {
      if(!window.confirm(
          'It looks like you\'ve typed a long message. Are you sure you want to leave the page?'
        )) {
        e.preventDefault();
      }
    }
...
function versionCheck() {
  var settings_page = (document.location.href.indexOf(
    'endoftheinter.net/loser.php?settings') != -1);
  if((!ch('alert_new_versions') || ch('last_version_check', 0) + 43200 >=
      start / 1000) && !settings_page) return;
  GM_xmlhttpRequest({
    method: 'GET',
    ...
    <a href="https://cosban.net/static/raw/greaseyeti.user.js">Update</a></td>
            ...
            $('table.greaseyeti_settings')
              .prepend(
                '<tr><td style="background: #88ff88; font-weight: bold; text-align: center; color: black; padding: 5px 0">Your version of GreaseYETI is up-to-date. Current version: ' +
    ...
    .each(function () {
      ...
      if(ch('remove_topics')) {
        if(isRemoved(topic_id)) {
          ...
          cells.eq(3)
            .append(' <a class="greaseyeti_nounderline_link rtopic ' +
              topic_id + '" title="Remove topic" style="color: #a33">X</a>');
        }
      }
      // Highlighted topics: highlight if necessary and add the link
      if(ch('highlight_topics')) {
        if(isHighlighted(topic_id)) cells.css('background-color', ch(
          'highlight_color', '#5cc'));
        cells.eq(3)
          .append(' <a class="greaseyeti_nounderline_link htopic ' +
            topic_id + '" title="Highlight topic" style="color: #3a3">H</a>');
      }
      ...
      if(ch('load_unread')) {
        ...
        if(ch('disable_topics')) {
          ...
          cells.eq(3)
            .append(' <a class="greaseyeti_nounderline_link dtopic ' +
              topic_id + '" title="Disable topic" style="color: #33a">D</a>');
        }
        if(add_to_unread && cells[2].getElementsByTagName('a')
          .length != 0)
          unread_topics.push(cells[2].getElementsByTagName('a')[0].href);
      }
      var topic_title = $(this)
        .children()
        .eq(0)
        .find('a')
        .eq(0)
...
function loadMoreTopics() {
  ...
  $('.grid')
    .append(
      '<tr><td colspan="4" id="gy_loading_row" style="text-align: center; padding: 5px 0; font-size: 11px;">Loading next page... <img src="' +
      load_gif + '" /></td></tr>');
  ...
  $.get(next_page, function (data) {
    ...
    var next_page_link = $(newPage)
      .find(
        'div.infobar a:contains("Next Page")');
...
function createLoadUnreadLink() {
  ...
  $('div.userbar')
    .append(
      ' | <a class="greaseyeti_link" style="text-decoration: underline" id="unread_topics_link">a</a>'
    );
}
...
function isRemoved(topic_id) {
  return(ch('removed_topics', false) && greaseyeti.removed_topics.indexOf(
    topic_id) != -1);
}

function isHighlighted(topic_id) {
  return(ch('highlighted_topics', false) && greaseyeti.highlighted_topics.indexOf(
    topic_id) != -1);
}

function isDisabled(topic_id) {
  return(ch('disabled_topics', false) && greaseyeti.disabled_topics.indexOf(
    topic_id) != -1);
}
...
function removeTopic() {
  ...
  if(confirm('Are you sure you want to remove the topic "' + topic_title +
    ...
    var topic_id = $(this)
      .attr('class')
      .replace(
        'greaseyeti_nounderline_link rtopic ', '');
...
function highlightTopic() {
  var topic_id = $(this)
    .attr('class')
    .replace(
      'greaseyeti_nounderline_link htopic ', '');
  if(isHighlighted(topic_id)) {
    greaseyeti.highlighted_topics.splice(greaseyeti.highlighted_topics.indexOf(
      topic_id), 1);
    $(this)
      .parent()
      .parent()
    ...
    $(this)
      ...
      .parent()
      .find('td')
      .css('background', ch('highlight_color',
        '#5cc'));
  }
  saveGreaseyeti(true);
}
...
function disableTopic() {
  var topic_id = $(this)
    .attr('class')
    .replace(
      'greaseyeti_nounderline_link dtopic ', '');
  var cells = $(this)
    .parent()
    .parent()
    .find('td');
  var unread_link = cells.eq(2)
    .find('a');
  if(unread_link.length == 2) {
    var unread_url = unread_link.eq(0)
      .attr('href');
  }
  if(isDisabled(topic_id)) {
    cells.css('opacity', '1.0');
    greaseyeti.disabled_topics.splice(greaseyeti.disabled_topics.indexOf(
      topic_id), 1);
...
function processPosts(page_number) {
  ...
  messages.each(function (i) {
    ...
    if($(this)
      .hasClass('greaseyeti_checked')) return;
    ignoratePost($(this));
...
function unreadMessagesInTitle() {
  ...
  if(!window_has_focus && unread_messages != 0) {
    document.title = '(' + unread_messages + ') ' + document.title.replace(
      /^\(\d+\) /, '');
  } else {
...
function numberPost(message_container, page_number, i) {
  ...
  message_container.children('.message-top')
    .append(' | #' + (i + 1 + 50 * (
      page_number - 1)));
}
...
function markEditedPosts(message_container) {
  ...
  message_container.children('.message-top a')
    .each(function () {
      if($(this)
        .text()
        .indexOf('edit') != -1 && $(this)
        ...
        .indexOf(
          'message.php') != -1) $(this)
        .css('color', ch('edited_color', 'red'));
    });
}

function stealthLogout() {
  if(!ch('stealth_logout')) return;
  $('a[href*="endoftheinter.net/logout.php"]')
    .click(function (e) {
...
function highlightPost(message_container) {
  message_container.find('.message-top')
    .each(function () {
      ...
      if(anon_topic) {
        ...
        if('Human #' + my_human_number == poster) message_top.css(
          'background-color', ch('my_color'));
      }
    });
}
...
function findMyHumanNumber() {
  ...
  var human_number_link = $(
    '.quickpost-body div:contains("You are posting as") a');
    ...
    .click(
      function () {
        ...
        $(this)
          .remove();
        tparent.append(
          '<span class="greaseyeti_loading">Loading original message... <img src="' +
          load_gif + '" /></span>');
        $.get(load_url, function (data) {
          var mssg = $(data)
            .find('td.message, div.message');
          mssg.find('script')
            .remove();
          ...
          mssg_html = mssg_html.substring(0, mssg_html.indexOf('---'))
            .replace(
              /^(\s|\<br\>)*/, '')
            .replace(/(\s|\<br\>)*$/, '');
          tparent.find('.greaseyeti_loading')
            .remove();
...
function checkWordLength(message_container) {
  ...
  message_container.find('.message')
    .contents()
    .each(function () {
...
function imgurLinks(message_container) {
  ...
  message_container.find('.message-body a')
    .each(function () {
      var anchor_link = $(this)
        .attr('href');
      var imgur_regexp =
        /^https?:(\/\/i\.imgur\.com\/[a-zA-Z0-9]+)\.(webm|gif|gifv)$/i;
      ...
      if(matches !== null) {
        ...
        $(this)
          .wrap('<div class="greaseyeti_imgur"></div>');
        if(file_extension === 'webm' || file_extension === 'gifv') {
          $(this)
            .before(
              '<video loop controls><source type="video/webm" src="' +
              imgur_base + '.webm"></source><source type="video/mp4" src="' +
              imgur_base + '.mp4"></source></video><br>');
        } else if(file_extension === 'gif') {
          $(this)
            .before('<img src="' + anchor_link + '"><br>');
        }
      }
    });
}
...
function autoscroll(post_count) {
  ...
  var current_y = $(document)
    .scrollTop() + $(window)
    .height();
  var last_post = $('.message-container')
    .last();
  var new_y = last_post.offset()
    .top + last_post.height();
  // If we're within the right distance of the bottom of the page, scroll it.
  if($(document)
    .height() < current_y + parseInt(ch('autoscroll_y'))) {
...
function changePage(page_number, new_page_number) {
  var current_y = $(document)
    .scrollTop() + $(window)
    .height();
  if(page_number + 1 == new_page_number && $(document)
    .height() < current_y +
    parseInt(ch('autoscroll_y'))) {
    if(document.location.href.match(/page=[0-9]+/gi)) {
      ...
      var newUrl = document.location.href + '&page=2' + (chat_mode_enabled ?
        '&chatmode=true' : '');
    }
    page_change = true;
...
function preserveMessage(topic_id) {
  ...
  $('a')
    .click(function () {
      ...
      var ahref = $(this)
        .prop('href');
      if(typeof ahref != 'undefined' && ahref.indexOf(
          'showmessages.php?topic=' + topic_id) != -1) {
        page_change = true;
        form_submitted = true;
        greaseyeti.preserved_message_text = $('textarea[name=message]')
          .val();
...
function restoreMessage(topic_id) {
  ...
  if(message.length != 0) $('textarea[name=message]')
    .val(message);
}

function findImages() {
  var new_images = $('.message .img-loaded')
    .not('.greaseyeti_img');
  new_images.each(function () {
    $(this)
      .addClass('greaseyeti_img');
    if($(this)
      .find('img')
      .attr('src')
      .indexOf('/i/t/') == -1) {
      ...
      $(this)
        .addClass('greaseyeti_img_collapsed');
    }
  });
}
...
function toggleImage(img) {
  ...
  if(img.hasClass('greaseyeti_img_expanded')) {
    ...
    if(expanded.length > 0) {
      ...
      img.after('<span class="greaseyeti_img_expanded"><img src="' + img.parent()
        .attr('imgsrc') + '" /></a>');
      parentAnchor.find('.greaseyeti_img_expanded img')
        .one('load', function () {
          resizeImage($(this)
            .parent(), shift_key);
        });
...
function expandAllImages() {
  ...
  if($(this)
    ...
    $('.greaseyeti_img_collapsed:visible')
      .click();
  } else {
    $('.greaseyeti_img_expanded:visible')
      .click();
...
function openSpoilersOption() {
  ...
  $('.userbar')
    .append(
      ' | <a id="open_spoilers" class="greaseyeti_link">Open All Spoilers</a>');
  $('#open_spoilers')
    .click(function () {
      $('.spoiler_on_close b')
        .click();
...
function chatModeOption() {
  ...
  $('.userbar')
    .append(
      ' | <a id="chat_mode" class="greaseyeti_link">Enable Chat Mode</a>');
  // Toggler
  $('#chat_mode')
    .click(function () {
      if(chat_mode_enabled) {
...
function filterMeOption() {
  ...
  var filter_me_url = '//boards.endoftheinter.net/showmessages.php?topic=' +
    topic_id + '&u=';
  var use_href = true;
  if(anon_topic) {
    findMyHumanNumber();
  ...
  if(use_href) {
    $('.userbar')
      .append(' | <a id="filter_me" class="greaseyeti_link" href="' +
        filter_me_url + '">Filter Me</a>');
  } else {
    $('.userbar')
      .append(
        ' | <a id="filter_me" class="greaseyeti_link">Filter Me</a>');
    $('#filter_me')
      .click(function () {
...
function addLikeAndRepeatOptions(message_container) {
  if((!ch('like_posts') && !ch('repeat_posts')) || document.location.href.indexOf(
      'archives.endoftheinter') != -1) return;
  if(ch('like_posts')) message_container.children('.message-top')
    .append(
      ' | <a class="greaseyeti_link like_post">Like</a>');
  if(ch('repeat_posts')) message_container.children('.message-top')
    .append(
      ' | <a class="greaseyeti_link repeat_post">Repeat</a>');
}
$(document)
  .on('click', '.like_post, .repeat_post', function () {
...
function populateHighlightedUsers() {
  ...
  var html = '<tr><td>' + getMyUsername() +
    '</td><td><input type="text" id="my_color" value="' +
    ch('my_color', '#c3c4c9;') +
    '" /></td><td><input type="checkbox" id="my_posts"' +
    settingCheck('my_posts') +
    '" /></td><td><input type="checkbox" id="my_topics"' +
    settingCheck('my_topics') +
    '" /></td><td><input type="checkbox" id="my_anon"' +
    settingCheck('my_anon') +
    '" /></td><td>&nbsp;</td></tr>';
  ...
  if(users != null) {
    for(var i = 0; i < users.length; i++) {
      html +=
        '<tr class="huser"><td><input type="text" class="highlight_username" value="' +
        users[i].username +
        '" /></td><td><input type="text" class="highlight_color" value="' +
        users[i].color +
        '" /></td><td><input type="checkbox" class="highlight_posts"' +
        checkForHighlight(users[i], 'posts') +
        ' /></td><td><input type="checkbox" class="highlight_topics"' +
        checkForHighlight(users[i], 'topics') +
...
function populateIgnoratedUsers() {
  ...
  if(users != null) {
    for(var i = 0; i < users.length; i++) {
      html +=
        '<tr class="iguser"><td><input type="text" class="ignorate_username" value="' +
        users[i].username +
        '" /></td><td><input type="text" class="ignorate_userid" value="' +
        users[i].userid +
...
function populateIgnoratedKWords() {
  ...
  if(words != null) {
    for(var i = 0; i < words.length; i++) {
      console.log(words[i]);
      html +=
        '<tr class="igkword"><td><input type="text" class="ignorate_kword" value="' +
        words[i].word +
        '" /></td><td><input type="checkbox" class="kw_ignorate_regex"' +
        checkforIgnorated(words[i], 'is_regex') +
        '" /></td><td><input type="checkbox" class="kw_ignorate_posts"' +
        checkforIgnorated(words[i], 'posts') +
...
function checkForHighlight(user, where) {
  return(user.hasOwnProperty(where) && user[where] === true ?
    ' checked="checked"' : '');
}

function checkforIgnorated(kword, where) {
  return(kword.hasOwnProperty(where) && kword[where] === true ?
    ' checked="checked"' : '');
}
