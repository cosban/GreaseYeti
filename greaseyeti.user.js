...
var load_gif = 'data:image/gif;base64,R0lGODlhEAAQAPIAAP///2Zm/9ra/o2N/mZm/6Cg/rOz/r29/iH/C05FVFNDQVBFMi4wAwEAAAAh/hpDcmVhdGVkIHdpdGggYWpheGxvYWQuaW5mbwAh+QQJCgAAACwAAAAAEAAQAAADMwi63P4wyklrE2MIOggZnAdOmGYJRbExwroUmcG2LmDEwnHQLVsYOd2mBzk' +
    'YDAdKa+dIAAAh+QQJCgAAACwAAAAAEAAQAAADNAi63P5OjCEgG4QMu7DmikRxQlFUYDEZIGBMRVsaqHwctXXf7WEYB4Ag1xjihkMZsiUkKhIAIfkECQoAAAAsAAAAABAAEAAAAzYIujIjK8pByJDMlFYvBoVjHA70GU7xSUJhmKtwHPAKzLO9HMaoKwJZ7Rf8AYPDDzKpZBqfvwQAIfkECQoAAAAs' +
    'AAAAABAAEAAAAzMIumIlK8oyhpHsnFZfhYumCYUhDAQxRIdhHBGqRoKw0R8DYlJd8z0fMDgsGo/IpHI5TAAAIfkECQoAAAAsAAAAABAAEAAAAzIIunInK0rnZBTwGPNMgQwmdsNgXGJUlIWEuR5oWUIpz8pAEAMe6TwfwyYsGo/IpFKSAAAh+QQJCgAAACwAAAAAEAAQAAADMwi6IMKQORfjdOe82' +
    'p4wGccc4CEuQradylesojEMBgsUc2G7sDX3lQGBMLAJibufbSlKAAAh+QQJCgAAACwAAAAAEAAQAAADMgi63P7wCRHZnFVdmgHu2nFwlWCI3WGc3TSWhUFGxTAUkGCbtgENBMJAEJsxgMLWzpEAACH5BAkKAAAALAAAAAAQABAAAAMyCLrc/jDKSatlQtScKdceCAjDII7HcQ4EMTCpyrCuUBjCYR' +
    'gHVtqlAiB1YhiCnlsRkAAAOwAAAAAAAAAAAA==';
var window_has_focus = false,
    currently_scrolling = false,
    uses_thumbnails = true,
    max_img_size = -1,
    shift_key = false,
    ctrl_key = false,
    highlighted_users = null,
    highlighted_colors = null,
    form_submitted = false,
    page_change = false,
    chat_mode_enabled = false;
// GRAB THE USER'S SETTINGS
var greaseyeti = JSON.parse(GM_getValue('greaseyeti', '-1'));
// if (greaseyeti == -1 && url.indexOf('loser.php?settings') == -1) document.location = '//endoftheinter.net/loser.php?settings';
// First, force HTTPS if enabled. There's no point loading the rest.
forceHttps();
// SETTINGS PAGE
if (url.indexOf('loser.php?settings') != -1) {
    if (typeof greaseyeti === 'number') greaseyeti = new Object();
    // DISPLAY SETTINGS - create some HTML.
    $('h1').html('GreaseYETI Settings');
    document.title = 'End of the Internet - GreaseYETI Settings';
    // Link to GreaseYeti topic
    var settings_html = '<tr><td>Need help? Have questions? Want to suggest a new feature? See the <a href="//boards.endoftheinter.net/showmessages.php?topic=8607569">GreaseYETI topic</a>.</td></tr>';
    // Stuff that applies everywhere
    settings_html += '<tr><th>Everywhere</th></tr><tr><td><input type="checkbox" id="dramalinks"' + settingCheck('dramalinks') + ' /> <label for="dramalinks">Add Dramalinks to current page</label><p class="desc">Adds Dramalinks to the top of each page. You will need to enable 3rd party cookies in your browser.</p></td></tr><tr><td><input type="checkbox" id="custom_titles"' + settingCheck('custom_titles') + ' /> <label for="custom_titles">Custom title format for pages</label><p class="desc">Allows you to use a custom format for the page title (or even shorten it). Enter in your custom title format below, where "%TITLE%" will be replaced by the actual page title (e.g. "GreaseYETI Settings"). For example, "End of the Internet - %TITLE%" is what LL uses by default.</p><p><input type="text" id="title_format" size="30" value="' + ch('title_format', 'End of the Internet - %TITLE%') + '" /></p></td></tr><tr><td><input type="checkbox" id="message_history"' + settingCheck('message_history') + ' /> <label for="message_history">Add link to old Message History to ETI bookmarks</label><p class="desc">Adds a link to your <a href="//boards.endoftheinter.net/history.php">Message History</a> in the top bar, at the end of your tag bookmarks.</p></td></tr><tr><td><input type="checkbox" id="alert_new_versions"' + settingCheck('alert_new_versions') + ' /> <label for="alert_new_versions">Alert me when GreaseYETI is updated</label></td></tr><tr><td><input type="checkbox" id="force_https"' + settingCheck('force_https') + ' /> <label for="force_https">Force ETI to use HTTPS</label><p class="desc">Automatically redirects you to HTTPS</p></td></tr><tr><td><input type="checkbox" id="custom_css"' + settingCheck('custom_css') + ' /> <label for="custom_css">Use custom CSS</label><p class="desc">Select this box to add custom CSS rules. These rules will apply on the core LL pages, but <b>not</b> the Wiki.</p><textarea id="custom_css_rules">' + ch('custom_css_rules') + '</textarea></td></tr>';
    // Topic list settings
    settings_html += '<tr><th>Topic Lists</th></tr><tr><td><input type="checkbox" id="autoload"' + settingCheck('autoload') + ' /> <label for="autoload">Autoload the next page of topics</label><p class="desc">Automatically load the next page of topics when you scroll near to the bottom.</p></td></tr><tr><td><input type="checkbox" id="remove_topics"' + settingCheck('remove_topics') + ' /> <label for="remove_topics">Option to remove topics</label><p class="desc">Creates a link next to the timestamp for each topic. When clicked, the topic is removed from the topic list permanently. Disabling this will bring back removed topics.</p></td></tr><tr><td><input type="checkbox" id="highlight_topics"' + settingCheck('highlight_topics') + ' /> <label for="highlight_topics">Option to highlight any topic</label><p class="desc">Creates a link next to the timestamp for each topic. When clicked, the topic is highlighted until clicked again. Enter in a highlight color below.</p><p><input type="text" id="highlight_color" value="' + ch('highlight_color', '#5cc') + '" /></p></td></tr><tr><td><input type="checkbox" id="load_unread"' + settingCheck('load_unread') + ' /> <label for="load_unread">Link to load unread topics</label><p class="desc">Creates a link to load all topics with new posts. When clicked, each topic opens in a new tab.</p><div><input type="checkbox" id="posted_only"' + settingCheck('posted_only') + ' /> <label for="posted_only">Only if viewing [Posted]</label><br /><input type="checkbox" id="unread_title"' + settingCheck('unread_title') + ' /> <label for="unread_title">Show number of unread topics in page title</label><br /><input type="checkbox" id="disable_topics"' + settingCheck('disable_topics') + ' /> <label for="disable_topics">Option to disable topics</label><p class="desc">Creates a link next to the timestamp for each topic. When clicked, the topic will no longer open in the "Load Unread Topics" link, and will be grayed out.</p></div></td></tr><tr><td><input type="checkbox" id="update_new_window"' + settingCheck('update_new_window') + ' /> <label for="update_new_window">Open topic update links in a new window/tab</label><p class="desc">Clicking the links in the Msgs column that show new posts since your last visit will open a new window</p></td></tr>';
    // Message list settings
    settings_html += '<tr><th>Message Lists</th></tr><tr><td><input type="checkbox" id="autoscroll"' + settingCheck('autoscroll') + ' /> <label for="autoscroll">Scroll the topic automatically when new posts appear</label><p class="desc">Scrolls the topic for you when new posts appear with Livelinks.</p><p><input type="text" size="8" id="autoscroll_y" value="' + ch('autoscroll_y', 400) + '" /> <label for="autoscroll_y">Maximum number of pixels from bottom to allow scroll</label></p><p class="desc">You must be within this distance from the bottom of the screen for the page to scroll.</p><p><input type="checkbox" id="page_change"' + settingCheck('page_change') + ' /> <label for="page_change">Change the page automatically</label></p></td></tr><tr><td><input type="checkbox" id="messages_in_title"' + settingCheck('messages_in_title') + ' /> <label for="messages_in_title">Change title when there are new posts</label><p class="desc">Adds a number in the title of the page in your browser tabs/window when there are new messages and the topic window does not have focus</label></td></tr><tr><td><input type="checkbox" id="stealth_logout"' + settingCheck('stealth_logout') + ' /> <label for="stealth_logout">Ask me to confirm logging out</label></td></tr><tr><td><input type="checkbox" id="load_omitted"' + settingCheck('load_omitted') + ' /> <label for="load_omitted">Load [quoted text omitted] by clicking</label><p class="desc">Clicking on "[quoted text omitted]" will load the actual message and insert it.</p></td></tr><tr><td><input type="checkbox" id="word_break"' + settingCheck('word_break') + ' /> <label for="word_break">Break page-stretching words into multiple lines</label><p class="desc">Forces long words to be broken into multiple lines to eliminate horizontal page-stretching. Enter the maximum number of characters a word can contain below.</p><p><input type="text" id="word_length" size="8" value="' + ch('word_length', 180) + '" /></p></td></tr><tr><td><input type="checkbox" id="expand_images"' + settingCheck('expand_images') + ' /> <label for="expand_images">Expand thumbnails with a click</label><p class="desc">Clicking on a thumbnail expands the image into its full-sized version</p><p><input type="checkbox" id="expand_all"' + settingCheck('expand_all') + ' /> <label for="expand_all">Create menu option to expand all thumbnails on a page</label></p></td></tr><tr><td><input type="checkbox" id="resize_images"' + settingCheck('resize_images') + ' /> <label for="resize_images">Resize large images to fit</label><p class="desc">Resizes images that would stretch the page. Shift-clicking an image will show it full-sized.</p></td></tr><tr><td><input type="checkbox" id="open_spoilers"' + settingCheck('open_spoilers') + ' /> <label for="open_spoilers">Userbar option to open all spoilers</label><p class="desc">Creates a link in the userbar that will open all spoilers on a page when clicked.</p></td></tr><tr><td><input type="checkbox" id="mark_edited"' + settingCheck('mark_edited') + ' /> <label for="mark_edited">Mark Message Detail link of edited posts</label><p class="desc">Colors the Message Detail link when a post has been edited. Enter in the color below for the text.</p><p><input type="text" id="edited_color" value="' + ch('edited_color', '#aa0000') + '" /></p></td></tr><tr><td><input type="checkbox" id="number_posts"' + settingCheck('number_posts') + ' /> <label for="number_posts">Number Posts</label></td></tr><tr><td><input type="checkbox" id="like_posts"' + settingCheck('like_posts') + ' /> <label for="like_posts">Add option to "Like" a post</label><p class="desc">Adds a "Like" option to the headers of each post.</p><p><input type="checkbox" id="like_automatic_post"' + settingCheck('like_automatic_post') + ' /> <label for="like_automatic_post">Automatically submit the post</label></p></td></tr><tr><td><input type="checkbox" id="repeat_posts"' + settingCheck('repeat_posts') + '" /><label for="repeat_posts">Add option to "Repeat" a post</label><p class="desc">Adds a "Repeat" option to the headers of every post. Clicking this automatically quotes and submits the post.</p></td></tr><tr><td><input type="checkbox" id="filter_me"' + settingCheck('filter_me') + ' /> <label for="filter_me">Create menu option to "Filter Me" in topics</label></td></tr><tr><td><input type="checkbox" id="imgur_integration"' + settingCheck('imgur_integration') + ' /> <label for="imgur_integration">Automatically embed .gif/.webm/.gifv from Imgur</label></td></tr><tr><td><input type="checkbox" id="chat_mode"' + settingCheck('chat_mode') + ' /> <label for="chat_mode">"Chat Mode" option</label><p class="desc">Creates a toggle that turns on "Chat Mode". Useful for topics where users post at a rapid rate, chat mode strips out most of the UI so you can keep up with the topic.</p></td></tr><tr><td><input type="checkbox" id="autoexpand_quickpost"' + settingCheck('autoexpand_quickpost') + ' /> <label for="autoexpand_quickpost">Automatically expand quickpost when you enter a topic</label></td></tr>';
    // Posting settings
    settings_html += '<tr><th>Posting</th></tr><tr><td><input type="checkbox" id="lastfm_integration"' + settingCheck('lastfm_integration') + ' /> <label for="lastfm_integration">Integrate Last.fm into your posts</label><p class="desc">Allows you to integrate Last.fm into your signature/posts. When submitting a post, %TRACK% will be replaced by "ARTIST ? TRACK", and %TIME% will let you know when the track was played.<p><input type="text" id="lastfm_username" size="25" value="' + ch('lastfm_username', '') + '" /> <label for="lastfm_username">Last.fm Username</label><br /><input type="text" id="lastfm_freq" size="5" value="' + ch('lastfm_freq', '') + '" /> <label for="lastfm_freq">Update Frequency (minutes)</label></p><p class="desc">After this many minutes, your Last.fm data will be refreshed.</p></td></tr><tr><td><input type="checkbox" id="ctrl_key_shortcuts"' + settingCheck('ctrl_key_shortcuts') + ' /> <label for="ctrl_key_shortcuts">CTRL + [KEY] shortcuts for inserting tags in posts</label><p class="desc">Allows you to use common CTRL + [KEY] combinations to insert tags into your posts. You can select text and tags will appear outside, or you can insert them through normal typing. The combinations are:<br />CTRL + B - bold<br />CTRL + I - italic<br />CTRL + U - underline<br />CTRL + S - spoiler<br />CTRL + M - spoiler with caption<br />CTRL + Q - quote<br />CTRL + P - preformatted<br />CTRL + G - mod</p></td></tr><tr><td><input type="checkbox" id="confirm_leaving"' + settingCheck('confirm_leaving') + ' /> <label for="confirm_leaving">Confirm leaving the page when I have a long message typed</label><p class="desc">Pops up a confirmation window if you are about to leave a page where you have a long message typed</p></td></tr><tr><td><input type="checkbox" id="preserve_messages"' + settingCheck('preserve_messages') + ' /> <label for="preserve_messages">Preserve messages when I change the page</label><p class="desc">Keeps your typed message when you change pages in a topic.</p></td></tr>';
    // Highlighting
    settings_html += '<tr><th>Highlighting</th></tr><tr><td><p class="desc">You can highlight posts and topics from specific users. Click "Add User" to create a new row, then fill in the username and a <a href="http://www.colorpicker.com/" target="_blank">CSS color</a>. Then select whether you would like to highlight posts and/or topics.</p><p class="desc">Your username is automatically added to the table to highlight your own content. You can also highlight your posts in [Anonymous] topics.</p><table id="highlighting"><tr style="font-weight: bold;"><td style="min-width: 25%">User</td><td style="min-width: 50%">Color</td><td>Posts</td><td>Topics</td><td>[Anonymous]</td><td>Remove</td></tr>' + populateHighlightedUsers() + '<tr><td colspan="6"><button id="add_highlight">Add User</button></table></td></tr>';
    // Ignorator
    settings_html += '<tr><th>Ignorator</th></tr><tr><td><p class="desc">You can ignore posts and topics from specific users. Click "Add User" to create a new row, then fill in the username or userid.</p><p class="desc">GreaseYeti will always ignore by userid and attempt to update usernames whenever possible.</p><table id="ignorating"><tr style="font-weight: bold;"><td style="min-width: 45%">User</td><td style="min-width: 45%">Userid</td><td>Remove</td></tr>' + populateIgnoratedUsers() + '<tr><td colspan="6"><button id="add_ignorate">Add User</button></table></td></tr>';
    // Save button
    settings_html += '<tr><th>Save</th></tr><tr><td style="text-align: center"><button id="gm_save">Save Settings</button><br /><span id="save_confirmation" style="font-weight: bold;"></span></td></tr>';
    // Apply the HTML and add some style
    $('.grid').html(settings_html);
    $('.grid label').css('font-weight', 'bold');
    $('.grid p, .grid div').css('margin', '0 0 0 25px');
    $('#custom_css_rules').css({
        'width': '90%',
        'margin': '10px auto',
        'height': '20em',
        'display': 'none',
...
if (document.location.href.indexOf('.net/topics/') != -1) {
    ...
    var page_number = 1,
        ...
        prevent_scroll_timeout;
    if (gup('page')) page_number = parseInt(gup('page'));
...
applyStyling();
addCustomCss();
if (gup('greaseyeti_debug') !== null) console.log(greaseyeti);
greaseyetiTime();
//
...
function ch(setting, default_value) {
    if (!greaseyeti.hasOwnProperty(setting)) return default_value;
...
function saveGreaseyeti(restrict_mutability) {
    if (typeof greaseyeti == 'number') return;
    // Mutable fields
    if (restrict_mutability) {
        var current_greaseyeti = JSON.parse(GM_getValue('greaseyeti', '-1'));
        if (typeof current_greaseyeti == 'number') return;
        var mutable_fields = [
            'lastfm_track',
            'lastfm_lastcheck',
            'lastfm_timestamp',
            'preserved_topic_id',
            'preserved_message_text',
            'removed_topics',
            'highlighted_topics',
            'disabled_topics',
            'last_version_check'
    ...
    GM_xmlhttpRequest({
        ...
        onload: function(responseDetails) {
            if (responseDetails.status == 200) {
                ...
                var color = dl.slice(dl.indexOf('{{') + 2).replace('}}', '');
                ...
                switch (color) {
                    ...
                    case 'kermit':
                        dl_bgcolor = '#000';
                        dl_textcolor = '#fff';
                        color = 'CODE KERMIT! <img alt=\'kermit.gif\' src=\'data:image/gif;base64,\' style=\'height: 2.1em; width: 2.1em; margin: 0.1em 0.5em; border: 1px #fff solid; float: left;\'/>';
                        break;
                    case 'dog':
                        dl_bgcolor = '#664422';
                        dl_textcolor = '#fff';
                        break;
                    case 'rainbow':
                        dl_bgcolor = 'linear-gradient(to right, rgba(255, 50, 50, 0.7), rgba(255, 160, 0, 0.7), rgba(255, 255, 100, 0.7), rgba(150, 255, 150, 0.7), rgba(120, 170, 255, 0.7), rgba(175, 0, 175, 0.7))';
                        dl_textcolor = '#000';
                        break;
                    default:
                ...
                $('.userbar').eq(0).before('<div id="dramalinks_ticker" style="padding: 4px"><strong>' + color + ':</strong>' + dl + '</div>');
...
function ctrlKeyShortcuts() {
    if (!ch('ctrl_key_shortcuts')) return;
    var open_tags = {
        'i': false,
        'b': false,
        'mod': false,
    ...
    $('textarea[name=message]').keydown(function(e) {
        ...
        switch (e.which) {
            ...
            case 85:
                ...
                break;
            case 77:
                if (!open_tags['spoiler']) var caption = prompt('Please enter a caption for your spoiler tag.');
                tag = 'spoiler';
                break;
            default:
                return;
        ...
        if (selectStart == selectEnd) {
            var ins = '<' + tag + '>';
            if (caption != null) ins = '<spoiler caption="' + caption + '">';
            if (open_tags[tag]) ins = '</' + tag + '>';
            ...
            var open_tag = tag;
            if (caption != null) open_tag = 'spoiler caption="' + caption + '"';
            $(this).val(v.substr(0, selectStart) + '<' + open_tag + '>' + v.substr(selectStart, selectEnd - selectStart) + '</' + tag + '>' + v.substr(selectEnd));
            $(this)[0].setSelectionRange(selectStart + 2 + open_tag.length, selectStart + 2 + open_tag.length + (selectEnd - selectStart));
        }
        e.preventDefault();
    });
}
...
function lastFMListeners() {
    if (!ch('lastfm_integration')) return;
    checkLastFM();
    $('input[value="Post Message"], input[value="Preview Message"]').click(function() {
        $(this).parents('form').find('textarea').val(function(i, v) {
            ...
            var now = new Date();
            var stamp = 'Just now';
            if (now - d > 5 * 60000) stamp = Math.floor((now - d) / 60000) + 'm ago';
            if (now - d > 3600000) stamp = Math.floor((now - d) / 3600000) + 'h ago';
            if (now - d > 86400000) stamp = Math.floor((now - d) / 86400000) + ' days ago';
            return v.replace('%TRACK%', ch('lastfm_track', 'None')).replace('%TIME%', stamp);
        });
    });
}

function checkLastFM() {
    if (ch('lastfm_username').length < 1) return;
...
function checkWindowClose(e) {
    if (ch('confirm_leaving') && !form_submitted && !page_change) {
        ...
        $('textarea[name=message]').each(function() {
            if ($(this).val().split(' ').length > 50) stop_it = true;
        });
        if (stop_it) {
...
function getHighlightColor(user, message_list) {
    if (highlighted_users === null) indexHighlightedUsers(message_list);
    var highlighted_index = highlighted_users.indexOf(user);
    if (highlighted_index == -1) return false;
...
function indexHighlightedUsers(message_list) {
    ...
    for (var i = 0; i < h_users.length; i++) {
        if (message_list && !h_users[i].posts) continue;
        if (!message_list && !h_users[i].topics) continue;
            ...
            if (responseDetails.status == 200) {
                ...
                if (settings_page) {
                    if (result.version > version_num) {
                        $('table.grid').prepend('<tr><td style="background: #ff8888; text-align: center; color: black; padding: 5px 0"><strong>A new version of\tGreaseYETI is out.</strong><br /><strong>Your version:</strong> ' + version_num.toFixed(2) + ' <strong>Current version:</strong> ' + result.version + '<br />' + result.changes + '<br /><a href="http://xatal.com/scripts/greaseyeti.user.js">Update</a></td></tr>');
                    } else {
                        $('table.grid').prepend('<tr><td style="background: #88ff88; font-weight: bold; text-align: center; color: black; padding: 5px 0">Your version of GreaseYETI is up-to-date. Current version: ' + result.version + '</td></tr>');
                    }
                } else if (result.version > version_num) {
                    $('body').prepend('<div style="position: absolute; top: 0; left: 0; right: 0; background: #ff8888; font-weight: bold; text-align: center; color: black; padding: 5px 0"><strong>A new version of GreaseYETI is out</strong> | <a href="http://xatal.com/scripts/greaseyeti.user.js">Update</a> | <a href="//endoftheinter.net/loser.php?settings">View Changes</a> | <a style="cursor: pointer; text-decoration: underline" id="remove_bar_yeti">Remove this damn bar</a></div>');
...
function processTopicList() {
    $('.grid tr:not(.gm_checked)').each(function() {
        ...
        if (ch('remove_topics')) {
            if (isRemoved(topic_id)) {
                ...
                cells.eq(3).append(' <a class="greaseyeti_nounderline_link rtopic ' + topic_id + '" title="Remove topic" style="color: #a33">X</a>');
            }
        }
        // Highlighted topics: highlight if necessary and add the link

        if (ch('highlight_topics')) {
            if (isHighlighted(topic_id)) cells.css('background-color', ch('highlight_color', '#5cc'));
            cells.eq(3).append(' <a class="greaseyeti_nounderline_link htopic ' + topic_id + '" title="Highlight topic" style="color: #3a3">H</a>');
        }
        ...
        if (ch('load_unread')) {
            ...
            if (ch('disable_topics')) {
                ...
                cells.eq(3).append(' <a class="greaseyeti_nounderline_link dtopic ' + topic_id + '" title="Disable topic" style="color: #33a">D</a>');
            }
            if (add_to_unread && cells[2].getElementsByTagName('a').length != 0) unread_topics.push(cells[2].getElementsByTagName('a')[0].href);
        }
...
function loadMoreTopics() {
    ...
    $('.grid').append('<tr><td colspan="4" id="gy_loading_row" style="text-align: center; padding: 5px 0; font-size: 11px;">Loading next page... <img src="' + load_gif + '" /></td></tr>');
    // AJAX request for the next page. Add each row to the table and reprocess the page. Bring back the event listener at the end and identify the next page.
    $.get(next_page, function(data) {
        ...
        processTopicList();
        var next_page_link = $(newPage).find('div.infobar a:contains("Next Page")');
...
function createLoadUnreadLink() {
    if (!ch('load_unread') || (ch('posted_only') && url.indexOf('Posted') == -1)) return;
    $('div.userbar').append(' | <a class="greaseyeti_link" style="text-decoration: underline" id="unread_topics_link">a</a>');
}
// Updates the Unread Topics link. If there aren't any, show a placeholder. If there are, create the link and possibly change the page title.
...
function updateUnreadTopics() {
    ...
    if (unread_topics.length > 0) {
        ...
        if (ch('unread_title')) document.title += ' (' + unread_topics.length + ')';
    } else {
...
// Helper functions for testing if a topic is removed, highlighted, or disabled. Need to make sure the arrays aren't blank before .indexOf!
...
function removeTopic() {
    ...
    if (confirm('Are you sure you want to remove the topic "' + topic_title + '"?')) {
        ...
        if (!greaseyeti.hasOwnProperty('removed_topics')) greaseyeti.removed_topics = [];
...
function highlightTopic() {
    ...
    if (isHighlighted(topic_id)) {
        ...
        if (!greaseyeti.hasOwnProperty('highlighted_topics')) greaseyeti.highlighted_topics = [];
...
function disableTopic() {
    ...
    if (isDisabled(topic_id)) {
        ...
        if (unread_link.length == 2) unread_topics.push(unread_url);
    } else {
        cells.css('opacity', '0.35');
        if (!greaseyeti.hasOwnProperty('disabled_topics')) greaseyeti.disabled_topics = [];
        greaseyeti.disabled_topics.push(topic_id);
        var unread_topic_index = unread_topics.indexOf(unread_url);
        if (unread_link.length == 2) unread_topics.splice(unread_topic_index, 1);
    }
    updateUnreadTopics();
    saveGreaseyeti(true);
}
...
function processPosts(page_number) {
    ...
    messages.each(function(i) {
        ...
        imgurLinks($(this));
        if (chat_mode_enabled) chatModePost($(this));
        if (!window_has_focus && !initial_load) unread_messages++;
...
function unreadMessagesInTitle() {
    if (!ch('messages_in_title')) return;
...
function highlightPost(message_container) {
    message_container.find('.message-top').each(function() {
        ...
        if (anon_topic) {
            findMyHumanNumber();
            if (my_human_number == 0 || !ch('my_anon')) return;
            if ('Human #' + my_human_number == poster) message_top.css('background-color', ch('my_color'));
        }
    });
}
...
function findMyHumanNumber() {
    if (my_human_number != -1) return;
    var human_number_link = $('.quickpost-body div:contains("You are posting as") a');
    if (human_number_link.length != 1) {
        my_human_number = 0;
        return;
    } else {
        my_human_number = human_number_link.text().replace('Human #', '');
    }
}

function identifyOmittedQuotes(message_container) {
    if (!ch('load_omitted')) return;
    message_container.find('.quoted-message a:contains("quoted text omitted")').click(function() {
        var load_url = $(this).attr('href');
        var tparent = $(this).parent();
        $(this).remove();
        tparent.append('<span class="greaseyeti_loading">Loading original message... <img src="' + load_gif + '" /></span>');
        $.get(load_url, function(data) {
            var mssg = $(data).find('td.message, div.message');
            mssg.find('script').remove();
            var mssg_html = mssg.html();
            mssg_html = mssg_html.substring(0, mssg_html.indexOf('---')).replace(/^(\s|\<br\>)*/, '').replace(/(\s|\<br\>)*$/, '');
    ...
    message_container.find('.message-body a').each(function() {
        ...
        if (matches !== null) {
            ...
            if (file_extension === 'webm' || file_extension === 'gifv') {
                $(this).before('<video loop controls><source type="video/webm" src="' + imgur_base + '.webm"></source><source type="video/mp4" src="' + imgur_base + '.mp4"></source></video><br>');
...
function autoscroll(post_count) {
    if (!ch('autoscroll') || currently_scrolling) return;
...
function changePage(page_number, new_page_number) {
    ...
    if (page_number + 1 == new_page_number && $(document).height() < current_y + parseInt(ch('autoscroll_y'))) {
        if (document.location.href.match(/page=[0-9]+/gi)) {
            var newUrl = document.location.href.replace(/page=[0-9]+/gi, 'page=' + (new_page_number)) + (chat_mode_enabled ? '&chatmode=true' : '');
        } else {
...
function preserveMessage(topic_id) {
    if (!ch('preserve_messages')) return;
    $('a').click(function() {
        page_change = false;
...
function restoreMessage(topic_id) {
    if (!ch('preserve_messages')) return;
    if (ch('preserved_topic_id') != topic_id) return;
    var message = ch('preserved_message_text');
    if (message.length != 0) $('textarea[name=message]').val(message);
}
...
function toggleImage(img) {
    ...
    if (img.hasClass('greaseyeti_img_expanded')) {
        if (shift_key) { // Shift to show the full size
            ...
            img.hide();
            parentAnchor.find('.greaseyeti_img_collapsed').show();
        }
...
function findMaxImageWidth() {
    max_img_size = Math.min(Math.min($(window).width(), $('div.body').width()) - 43 - $('.userpic').eq(0).width(), $('.message').eq(0).width()) - 5;
}

function openSpoilersOption() {
    if (!ch('open_spoilers')) return;
...
function enableChatMode() {
    ...
    // Add CSS rules
    $('body').before('<style type="text/css"> \t\tbody.chat_mode td.userpic * {height: 45px; width: auto;} \t\tbody.chat_mode td.userpic center {display: none} \t\tbody.chat_mode div.body > small {display: none} \t\tbody.chat_mode .message div + br {display: none} \t\tbody.chat_mode div.body > br {display: none} \t\tbody.chat_mode form.quickpost { height: 200px; } \t\tbody.chat_mode form.quickpost textarea { height: 100px;} \t');
...
function addLikeAndRepeatOptions(message_container) {
    if ((!ch('like_posts') && !ch('repeat_posts')) || document.location.href.indexOf('archives.endoftheinter') != -1) return;
    if (ch('like_posts')) message_container.children('.message-top').append(' | <a class="greaseyeti_link like_post">Like</a>');
    if (ch('repeat_posts')) message_container.children('.message-top').append(' | <a class="greaseyeti_link repeat_post">Repeat</a>');
}
$(document).on('click', '.like_post, .repeat_post', function() {
    ...
    if (this_was_a_like) {
        ...
        if (poster_username.indexOf('#') != -1) my_username = 'This human';
        // Create the post text
        var post_text = '<img src="http://i4.endoftheinter.net/i/n/f818de60196ad15c888b7f2140a77744/like.png" /> ' + my_username + ' likes ' + poster_username + '\'s post';
...
function populateHighlightedUsers() {
    // First add yourself
    var html = '<tr><td>' + getMyUsername() + '</td><td><input type="text" id="my_color" value="' + ch('my_color', '#c3c4c9;') + '" /></td><td><input type="checkbox" id="my_posts"' + settingCheck('my_posts') + '" /></td><td><input type="checkbox" id="my_topics"' + settingCheck('my_topics') + '" /></td><td><input type="checkbox" id="my_anon"' + settingCheck('my_anon') + '" /></td><td>&nbsp;</td></tr>';
    // Then list everyone you've added
    var users = ch('highlighted_users', null);
    if (users != null) {
        for (var i = 0; i < users.length; i++) {
            html += '<tr class="huser"><td><input type="text" class="highlight_username" value="' + users[i].username + '" /></td><td><input type="text" class="highlight_color" value="' + users[i].color + '" /></td><td><input type="checkbox" class="highlight_posts"' + checkForHighlight(users[i], 'posts') + ' /></td><td><input type="checkbox" class="highlight_topics"' + checkForHighlight(users[i], 'topics') + ' /></td><td>&nbsp;</td><td class="highlight_remove"><a style="color: #a33">X</a></td></tr>';
        }
    }
    return html;
}
